...
const CategoriesMenu = () => {
  ...
  const handleClose = (categoryId) => {
    navigate(`/t2/${shopInfo.sub_domain}/products`, {
      state: { keys: { category_id: categoryId } },
    });
    setAnchorEl(null);
  };
  ...
  const GetSubCategories = (sizeParams = 200) => {
    GetSubCategoryRequest({
      ...
      onSuccess: (res) => {
        const filteredSubCategories = res.data.results.filter(
      ...
      >
        ...
        {categories?.results?.length > 4 && (
          <Box sx={{ position: "relative", mr: 2 }}>
            ...
            ></BoxMenuSkew>
            <ButtonStyle onClick={handleMoreClick} sx={{ fontFamily: "Cairo" }}>
              {t("More")}
            </ButtonStyle>
          </Box>
        )}
        <Box sx={{ position: "relative", mr: 3 }}>
          <BoxMenuSkew
            sx={{ bgcolor: theme.palette.primary.red }}
          ></BoxMenuSkew>
          <ButtonStyle
            sx={{ fontFamily: "Cairo" }}
            onClick={() => navigate(`/t2/${shopInfo?.sub_domain}/discounts`)}
          >
            {t("Sales")}
          </ButtonStyle>
        </Box>
        ...
        >
          {GetSubCategoryResponse.isPending ? (
            ...
            subCategories?.map((subcategory, index) => (
              <MenuItem
                key={index}
                onClick={() => handleClose(subcategory.id)}
                sx={{ fontFamily: "Cairo" }}
              >
                {subcategory.name}
              </MenuItem>
